...
=======
    ...
    tensor_2 = torch.autograd.Variable(
        torch.FloatTensor([2.0]), requires_grad=False
    ).to(data.device)
    if symmetric:
    ...
    if method_v == QuantizeMethod.FIX_NONE:
        ...
        if range_method_v == RangeMethod.RANGE_MAX:
            if float_scale:
                ...
                new_scale = torch.pow(
                    ...
                    torch.ceil(
                        torch.log(max_data)
                        / torch.FloatTensor([1]).fill_(np.log(2.0)).to(max_data.device)
                    ),
                )

                scale.data = new_scale
                return _do_quantize(
                    data, scale, bitwidth, stochastic=stochastic, group=group
                )
                ...
                torch.ceil(
                    torch.log(
                        torch.max(
                            ...
                            torch.topk(torch.abs(data.view(-1)), data.nelement() // 10)[
                                0
                            ][-1],
            ...
            scale.data = new_scale
            return _do_quantize(
                data, scale, bitwidth, stochastic=stochastic, symmetric=not zero_point
            )
            ...
            temp_scale = torch.ceil(
                torch.log(
                    torch.max(
                        torch.max(abs(data)), torch.tensor(EPS).float().to(data.device)
                    )
                )
                / np.log(2.0)
            )
            for i in range(SWEEP):
                errors[i] = torch.abs(
                    _do_quantize(data, temp_scale - i, bitwidth)[0] - data
                ).sum()
        ...
        if float_scale:
            if zero_point:
                ...
                scale.data = new_scale
                return _do_quantize(
                    data, scale, bitwidth, stochastic=stochastic, symmetric=False
                )
            else:
                scale.data = max_data
                return _do_quantize(
                    data, scale, bitwidth, stochastic=stochastic, symmetric=True
                )
        else:
